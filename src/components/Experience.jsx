...
export default function Experience() {
  return (
    <div className="border-b border-gray-800 pb-4" id="experience">
      <h1 className="my-20 text-center text-5xl arimo"> Experience</h1>
      <div>
        {EXPERIENCES.map((experience, index) => (
          <div key={index} className="mb-8 flex flex-wrap lg:justify-center">
            <motion.div initial={{ opacity: 0, x: -100 }}
              ...
              <p className="mb-2 text-sm font-nunito text-neutral-400">{experience.year}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 100 }}
              whileInView={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6, delay: index * 0.2 }} className="w-full max-w-xl lg:w-3/4">
              <h6 className="mb-2 font-nunito font-semibold">
                {experience.role} -{" "}
                <span className="text-sm font-nunito text-purple-600">{experience.company}</span>
              </h6>
              <p className="mb-4 font-nunito text-neutral-400">{experience.description}</p>
